...
const starts = async (sesName) => {
    try {
        ...
        client.on('CB:Presence', asd => {
            ...
            Client.sendText(asd.id, `@${asd.participant.split('@')[0]} Cie mengetik! Gajadi AFK Deh><`)
                }
        })
        ...
        client.on('new-msg', (message) => {
			...
			if (global.tebakgambar[from] && global.tebakgambar[from].id && global.tebakgambar[from].jawaban.toLowerCase() == body.toLowerCase()) Client.reply(from, `Berhasil dijawab oleh @${sender.split("@")[0]}`, message).then(() => global.tebakgambar[from] = {}) 
